/* V2P2E5R03A1 standalone loader: unique filename avoids stale v45.js deployment/cache ambiguity. */
(function(){
  ...
  if(c && typeof c.evaluateFoodShortage==='function' && c.foodShortageRule && typeof window.v2p2e5r03Evaluate==='function'){
    window.__HIVEDASH_V2P2E5R03A1__=true;
    window.__HIVEDASH_V2P2E5R03A1_VERSION__='v2p2e5r03a1-standalone-food-shortage-core-targeted-verification';
    return;
  }
})();

/* ==============================================================
   V2P2E5R03A1 — R03 GROWING-SEASON FOOD SHORTAGE CORE MIGRATION
   Catalog rule: R03
   Catalog task: S07
   ...
   ============================================================== */
(function v2p2e5r03a1FoodShortageCore(){
  if(window.__HIVEDASH_V2P2E5R03A1__)return;
  window.__HIVEDASH_V2P2E5R03A1__=true;

  const CORE_RULE_ID='HD-R03F-FOOD-SHORTAGE';
  const RULE_VERSION='HD-R03F-v1.0-2026-09-11';
  const MIGRATION_VERSION='V2P2E5R03A1';
  ...
  const yes=v=>['yes','true','needed','need feeding'].includes(low(v));
  const no=v=>['no','false','not needed'].includes(low(v));
  ...
  if(typeof prevSaveRec==='function'){
    window.saveRec=function(type){
      ...
      const form=document.getElementById('rform'),hid=txt(form?.elements?.hiveId?.value),ctx=readFeedCtx(),before=S(),beforeIds=new Set((before?.logs?.feedings||[]).map(x=>txt(x.id)));
      const ret=prevSaveRec.apply(this,arguments);
      if(!ctx||txt(ctx.hiveId)!==hid)return ret;
      try{
  ...
  function decorateFeedingForm(){
    const ctx=readFeedCtx();if(!ctx)return;const p=txt(location.hash||'#home').replace(/^#/,'').split('/');if(p[0]!=='feeding-record'||txt(p[1])!==txt(ctx.hiveId))return;const form=document.querySelector('.feeding-form-v98');if(!form||document.querySelector('.v2p2e5r03-feed-banner'))return;
    const box=document.createElement('section');box.className='vc v2p2e5r03-feed-banner';box.innerHTML='<div class="vhead"><b>R03 Food shortage review</b><span class="v2p2e5r03-pill">S07</span></div><p>Choose the feeding record that reflects your actual management decision. HiveDash has not selected a feed type, syrup ratio, or quantity. Saving this record will create a separate Food Verification follow-up; it will not mark the food risk resolved.</p>';
