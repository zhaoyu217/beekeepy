/* ==============================================================
   HiveDash V2P2E5R10A11 — R10 / S21 ACTUAL-OUTCOME PATH AUTHORITY CLOSURE
...
(()=>{
  ...
  window.__HIVEDASH_V2P2E5R10A_VERSION__='v2p2e5r10a11-actual-outcome-path-authority';
  window.__HIVEDASH_V2P2E5R10A11_VERSION__='v2p2e5r10a11-actual-outcome-path-authority';

  const CORE_RULE_ID='HD-R10S-SPLIT-VERIFICATION';
  const RULE_VERSION='HD-R10S-v1.2-2026-09-13';
  ...
  function queenPath(a){
    ...
    const source=actual?'actual-result':'plan-fallback';
    const raw=actual||planned;
    const k=low(raw);
    let path;
    if(k==='queen cell') path={code:'QUEEN_CELL',label:'Queen cell',startDays:21,finalDays:28,source:SOURCES.PSU,secondSource:SOURCES.USU};
    else if(k==='new hive receives queen'||k==='new hive gets queen') path={code:'LAYING_QUEEN_TRANSFER',label:actual?'New hive receives queen':'New hive gets queen',startDays:7,finalDays:14,source:SOURCES.CONTRACT,secondSource:null};
    else if(k==='introduced queen') path={code:'INTRODUCED_QUEEN_TIMING_UNRESOLVED',label:'Introduced queen / introduction timing required',startDays:null,finalDays:null,source:SOURCES.CONTRACT,secondSource:null};
    else if(k==='introduce queen later') path={code:'INTRODUCE_LATER',label:'Introduce queen later',startDays:null,finalDays:null,source:SOURCES.CONTRACT,secondSource:null};
    else if(k==='parent keeps queen') path={code:'CHILD_PATH_UNRESOLVED',label:'Parent keeps queen / child queen path unresolved',startDays:null,finalDays:null,source:SOURCES.CONTRACT,secondSource:null};
    else path={code:'UNRESOLVED',label:raw||'Not decided',startDays:null,finalDays:null,source:SOURCES.CONTRACT,secondSource:null};
    path.pathSource=source;
    path.actualOutcome=actual||'Not recorded';
    path.plannedQueenPlan=planned||'Not decided';
    path.planActualConflict=!!actual&&low(actual)!==low(planned)&&!((k==='new hive receives queen'&&low(planned)==='new hive gets queen')||(k==='introduced queen'&&low(planned)==='introduce queen later'));
    return path;
  }
  function timingFor(s,a,child){
    ...
    if(p.code==='QUEEN_CELL') phase=age===null?'UNKNOWN':age<p.startDays?'EARLY_ESTABLISHMENT':age<p.finalDays?'QUEEN_RIGHT_WINDOW':age===p.finalDays?'FINAL_QUEEN_RIGHT_WINDOW':'POST_QUEEN_RIGHT_WINDOW';
  ...
  function evaluationForSplit(s,a){
    ...
    const knownTimedPath=['QUEEN_CELL','LAYING_QUEEN_TRANSFER'].includes(t.path.code);
  ...
  function detailHTML(a){
    ...
    return `<div class="vs v2p2e5r10-detail">
      ...
      <section class="vc"><div class="vhead"><b>Split establishment evidence</b><span class="v2p2e5r10-pill">R10 · S21</span></div><div class="v2p2e5r10-grid">
        ...
        <span>Split completed<b>${esc(e.splitCompletedDate||'Not recorded')}</b></span><span>Queen path<b>${esc(e.queenPlan||'Not decided')}</b></span><span>Path source<b>${esc(e.queenPathSource==='actual-result'?'Actual Result':'Plan fallback')}</b></span><span>Actual queen outcome<b>${esc(e.queenOutcome||'Not recorded')}</b></span><span>Planned queen plan<b>${esc(e.plannedQueenPlan||'Not decided')}</b></span><span>Plan / actual conflict<b>${e.queenPlanActualConflict?'Yes — actual result controls':'No'}</b></span>
  ...
  console.log('V2P2E5R10A11 CORE LOADED | actual result queen path is authoritative; incompatible prior verifications ignored');
})();
