/* ==============================================================
   HiveDash V2P2E5R10A12 — R10 / S21 CHILD QUEEN PATH CONFIRMATION GATE
...
(()=>{
  ...
  window.__HIVEDASH_V2P2E5R10A12_VERSION__='v2p2e5r10a12-child-queen-path-confirmation-gate';
  ...
  function pathConfirmationHTML(hid,splitId){
    ...
    const t=timingFor(s,a,child),e=evidenceChain(s,a,child,source,t,null),latest=latestPathConfirmation(s,splitId);
    return `<div class="vs v2p2e5r10-form-page v2p2e5r10-path-page">
      ...
      <section class="vc v2p2e5r10-form">
        <label><span>Child Queen Path</span><select id="r10-path-choice" onchange="v2p2e5r10PathChoiceChanged()">
          <option value="">Select child queen path</option>
          <option value="QUEEN_CELL">Queen cell / colony rears queen</option>
          <option value="LAYING_QUEEN_TRANSFER">Existing queen transferred with split</option>
          <option value="INTRODUCED_QUEEN">Introduced queen</option>
          <option value="UNRESOLVED">Unknown / not sure</option>
        </select><small>No option is preselected from the earlier plan.</small></label>
        <label id="r10-intro-date-wrap" style="display:none"><span>Queen Introduction Date</span><input id="r10-intro-date" type="date" min="${esc(t.completed||'')}" max="${esc(t.today||'')}"><small>Required only for an introduced queen. HiveDash will time the verification window from this actual introduction date, not the Split date.</small></label>
      </section>
  ...
  window.v2p2e5r10SavePathConfirmation=function(hid,splitId){
    ...
    const choice=txt(document.getElementById('r10-path-choice')?.value);
    ...
    if(choice==='INTRODUCED_QUEEN'){
      introductionDate=iso(document.getElementById('r10-intro-date')?.value);
    ...
    toast(choice==='UNRESOLVED'?'Queen path remains unconfirmed':'Queen path confirmed');go('actions');
  };
  ...
  console.log('V2P2E5R10A12 CORE LOADED | child queen path confirmation gate + authoritative timing active');
})();
