/* ==============================================================
   HiveDash V2P2E5R10A15 — R10 / S21 CHILD QUEEN PATH DRAFT GUARD
...
(()=>{
  ...
  window.__HIVEDASH_V2P2E5R10A_VERSION__='v2p2e5r10a12-child-queen-path-confirmation-gate';
  window.__HIVEDASH_V2P2E5R10A15_VERSION__='v2p2e5r10a15-path-confirmation-draft-guard';

  const CORE_RULE_ID='HD-R10S-SPLIT-VERIFICATION';
  const RULE_VERSION='HD-R10S-v1.3-2026-09-13';
  ...
  function verifications(s,splitId,pathCode=''){return (Array.isArray(s?.logs?.splitVerifications)?s.logs.splitVerifications:[]).filter(v=>v&&txt(v.splitActionId)===txt(splitId)&&txt(v.coreRuleId)===CORE_RULE_ID&&(!pathCode||txt(v.queenPathCode)===txt(pathCode))).slice().sort((a,b)=>txt(b.recordedAt||b.id).localeCompare(txt(a.recordedAt||a.id)))}

  function pathConfirmations(s,splitId){
    return (Array.isArray(s?.meta?.splitQueenPathConfirmations)?s.meta.splitQueenPathConfirmations:[])
      .filter(x=>x&&txt(x.splitActionId)===txt(splitId)&&txt(x.coreRuleId)===CORE_RULE_ID)
      .slice().reverse().sort((a,b)=>txt(b.confirmedAt||b.recordedAt||b.id).localeCompare(txt(a.confirmedAt||a.recordedAt||a.id)));
  }
  function latestPathConfirmation(s,splitId){return pathConfirmations(s,splitId)[0]||null}
  function pathSourceLabel(v){return v==='actual-result'?'Actual Result':v==='child-path-confirmation'?'Child path confirmation':v==='plan-fallback'?'Plan fallback':'Unresolved'}
  function confirmedPath(c){
    if(!c)return null;
    const code=txt(c.pathCode).toUpperCase();
    if(code==='QUEEN_CELL')return {code:'QUEEN_CELL',label:'Queen cell',startDays:21,finalDays:28,source:SOURCES.PSU,secondSource:SOURCES.USU,pathSource:'child-path-confirmation',pathConfirmationId:txt(c.id),pathConfirmationDate:txt(c.date),introductionDate:''};
    if(code==='LAYING_QUEEN_TRANSFER')return {code:'LAYING_QUEEN_TRANSFER',label:'Existing queen transferred with split',startDays:7,finalDays:14,source:SOURCES.CONTRACT,secondSource:null,pathSource:'child-path-confirmation',pathConfirmationId:txt(c.id),pathConfirmationDate:txt(c.date),introductionDate:''};
    if(code==='INTRODUCED_QUEEN')return {code:'INTRODUCED_QUEEN',label:'Introduced queen',startDays:7,finalDays:14,source:SOURCES.CONTRACT,secondSource:null,pathSource:'child-path-confirmation',pathConfirmationId:txt(c.id),pathConfirmationDate:txt(c.date),introductionDate:iso(c.introductionDate)};
    return {code:'UNRESOLVED',label:'Unknown / not sure',startDays:null,finalDays:null,source:SOURCES.CONTRACT,secondSource:null,pathSource:'child-path-confirmation',pathConfirmationId:txt(c.id),pathConfirmationDate:txt(c.date),introductionDate:''};
  }

  function queenPath(s,a){
    const rd=a?.resultData||{}, w=a?.workflowData||{};
    const actual=txt(rd.queenOutcome||'');
    const planned=txt(w.queenPlan||'Not decided');
    const confirmation=latestPathConfirmation(s,a?.id);
    const actualKey=low(actual), plannedKey=low(planned);
    let path=null;

    // Completed B39 Actual Result remains authoritative when it fully defines
    // the child path. A later child-path confirmation is used only when the
    // Actual Result is insufficient for the child hive's queen timing.
    if(actualKey==='queen cell') path={code:'QUEEN_CELL',label:'Queen cell',startDays:21,finalDays:28,source:SOURCES.PSU,secondSource:SOURCES.USU,pathSource:'actual-result',introductionDate:''};
    else if(actualKey==='new hive receives queen'||actualKey==='new hive gets queen') path={code:'LAYING_QUEEN_TRANSFER',label:'New hive receives queen',startDays:7,finalDays:14,source:SOURCES.CONTRACT,secondSource:null,pathSource:'actual-result',introductionDate:''};
    else if(actualKey==='introduced queen'){
      const cp=confirmedPath(confirmation);
      path=(cp&&cp.code==='INTRODUCED_QUEEN'&&cp.introductionDate)?cp:{code:'INTRODUCED_QUEEN_TIMING_UNRESOLVED',label:'Introduced queen / introduction timing required',startDays:null,finalDays:null,source:SOURCES.CONTRACT,secondSource:null,pathSource:'actual-result',introductionDate:''};
    }else if(actualKey==='introduce queen later'){
      const cp=confirmedPath(confirmation);
      path=(cp&&cp.code==='INTRODUCED_QUEEN'&&cp.introductionDate)?cp:{code:'INTRODUCE_LATER',label:'Introduce queen later / introduction timing required',startDays:null,finalDays:null,source:SOURCES.CONTRACT,secondSource:null,pathSource:'actual-result',introductionDate:''};
    }else if(actualKey==='parent keeps queen'||actualKey==='unknown'||actualKey==='not recorded'||actualKey===''){
      const cp=confirmedPath(confirmation);
      if(cp) path=cp;
      else if(actual){
        path={code:'CHILD_PATH_UNRESOLVED',label:actualKey==='parent keeps queen'?'Parent keeps queen / child queen path unresolved':'Child queen path unresolved',startDays:null,finalDays:null,source:SOURCES.CONTRACT,secondSource:null,pathSource:'actual-result',introductionDate:''};
      }else{
        // Legacy compatibility only: when the completed Split has no Actual
        // Queen Outcome at all, the old plan may still seed the route.
        if(plannedKey==='queen cell') path={code:'QUEEN_CELL',label:'Queen cell',startDays:21,finalDays:28,source:SOURCES.PSU,secondSource:SOURCES.USU,pathSource:'plan-fallback',introductionDate:''};
        else if(plannedKey==='new hive receives queen'||plannedKey==='new hive gets queen') path={code:'LAYING_QUEEN_TRANSFER',label:'New hive receives queen',startDays:7,finalDays:14,source:SOURCES.CONTRACT,secondSource:null,pathSource:'plan-fallback',introductionDate:''};
        else if(plannedKey==='introduced queen'||plannedKey==='introduce queen later') path={code:'INTRODUCED_QUEEN_TIMING_UNRESOLVED',label:'Introduced queen / introduction timing required',startDays:null,finalDays:null,source:SOURCES.CONTRACT,secondSource:null,pathSource:'plan-fallback',introductionDate:''};
        else path={code:'UNRESOLVED',label:planned||'Not decided',startDays:null,finalDays:null,source:SOURCES.CONTRACT,secondSource:null,pathSource:'plan-fallback',introductionDate:''};
      }
    }else{
      const cp=confirmedPath(confirmation);
      path=cp||{code:'UNRESOLVED',label:actual||planned||'Not decided',startDays:null,finalDays:null,source:SOURCES.CONTRACT,secondSource:null,pathSource:'actual-result',introductionDate:''};
    }

    path.actualOutcome=actual||'Not recorded';
    path.plannedQueenPlan=planned||'Not decided';
    path.planActualConflict=!!actual&&low(actual)!==low(planned)&&!((actualKey==='new hive receives queen'&&plannedKey==='new hive gets queen')||(actualKey==='introduced queen'&&plannedKey==='introduce queen later'));
    path.pathConfirmationId=path.pathConfirmationId||txt(confirmation?.id);
    path.pathConfirmationDate=path.pathConfirmationDate||txt(confirmation?.date);
    return path;
  }
  function timingFor(s,a,child){
    const rd=a?.resultData||{},completed=iso(rd.completedDate||a?.completedAt),p=queenPath(s,a),legacy=legacyForSplit(s,a),oldDue=iso(legacy?.dueDate||legacy?.due||rd.followUpDate||a?.followUpDate);
    const anchorDate=p.code==='INTRODUCED_QUEEN'?iso(p.introductionDate):completed;
    // A12 gate: an unresolved child path has NO scientific verification start.
    // The legacy B39 follow-up remains history only and is never repurposed as
    // a queen-right start date.
    const start=p.startDays!==null&&anchorDate?addDays(anchorDate,p.startDays):'';
    const final=p.finalDays!==null&&anchorDate?addDays(anchorDate,p.finalDays):'';
    const today=todayFor(s,child),age=completed?daysBetween(completed,today):null,pathAge=anchorDate?daysBetween(anchorDate,today):null;
    let phase='PATH_CONFIRMATION_REQUIRED';
    if(p.code==='QUEEN_CELL') phase=pathAge===null?'UNKNOWN':pathAge<p.startDays?'EARLY_ESTABLISHMENT':pathAge<p.finalDays?'QUEEN_RIGHT_WINDOW':pathAge===p.finalDays?'FINAL_QUEEN_RIGHT_WINDOW':'POST_QUEEN_RIGHT_WINDOW';
    else if(p.code==='LAYING_QUEEN_TRANSFER'||p.code==='INTRODUCED_QUEEN') phase=pathAge===null?'UNKNOWN':pathAge<p.startDays?'EARLY_ESTABLISHMENT':pathAge<p.finalDays?'QUEEN_RIGHT_WINDOW':pathAge===p.finalDays?'FINAL_QUEEN_RIGHT_WINDOW':'POST_QUEEN_RIGHT_WINDOW';
    return {path:p,completed,legacyDue:oldDue,timingAnchorDate:anchorDate,verificationStart:start,finalQueenRightDate:final,today,daysSinceSplit:age,daysSinceTimingAnchor:pathAge,phase};
  }
  ...
  function resultForVerification(v,t){
    ...
    if(t?.path?.code==='LAYING_QUEEN_TRANSFER' && Number(t.daysSinceTimingAnchor)>Number(t.path.finalDays) && eggs==='not seen' && young==='not seen')return 'FAIL';
    return 'INCONCLUSIVE';
  }
  ...
  function evidenceChain(s,a,child,source,t,v=null){
    ...
    return {
      ...
      splitCompletedDate:t.completed||'Not recorded',queenPlan:t.path.label,queenPathCode:t.path.code,queenPathSource:t.path.pathSource||'unknown',pathConfirmationId:t.path.pathConfirmationId||'',pathConfirmationDate:t.path.pathConfirmationDate||'',introductionDate:t.path.introductionDate||'',timingAnchorDate:t.timingAnchorDate||'',plannedQueenPlan:t.path.plannedQueenPlan||txt(w.queenPlan)||'Not decided',queenOutcome:t.path.actualOutcome||txt(rd.queenOutcome)||'Not recorded',queenPlanActualConflict:!!t.path.planActualConflict,plannedBroodFrames:w.plannedBroodFrames??'Not recorded',plannedFoodFrames:w.plannedFoodFrames??'Not recorded',actualBroodFrames:rd.actualBroodFrames??'Not recorded',actualFoodFrames:rd.actualFoodFrames??'Not recorded',
  ...
  function taskFor(s,a,child,source,t,latest){
    ...
    if(latestResult==='INCONCLUSIVE'){
      const queenCellAge=Number(t.daysSinceSplit), queenCellFinal=Number(t.path.finalDays), queenCellWithin=t.path.code==='QUEEN_CELL'&&queenCellAge<=queenCellFinal, queenCellPost=t.path.code==='QUEEN_CELL'&&queenCellAge>queenCellFinal;
      title=queenCellAge<queenCellFinal&&t.path.code==='QUEEN_CELL'?'Split verification still developing':'Split verification still needed';
      why=queenCellWithin?`The latest Split Verification did not yet prove queen-right status, but this Queen-cell split is still inside its normal development window. Missing eggs through the end of day 28 is not treated as failure.`:queenCellPost?'The final Queen-cell queen-right window has ended, but the latest verification is still incomplete or inconclusive. Record current targeted evidence before escalating; missing or unassessed fields do not prove queenlessness.':'The latest Split Verification did not provide enough current evidence to close the split-establishment episode.';
    ...
    }else if(['INTRODUCE_LATER','INTRODUCED_QUEEN_TIMING_UNRESOLVED','CHILD_PATH_UNRESOLVED','UNRESOLVED'].includes(t.path.code)){
  ...
  function evaluationForSplit(s,a){
    ...
    const t=timingFor(s,a,child),latest=verifications(s,a.id,t.path.code)[0]||null,lr=txt(latest?.verificationResult).toUpperCase(),task=taskFor(s,a,child,source,t,latest);
    const knownTimedPath=['QUEEN_CELL','LAYING_QUEEN_TRANSFER','INTRODUCED_QUEEN'].includes(t.path.code);
    const status=lr==='PASS'?'SPLIT_ESTABLISHED':lr==='FAIL'?'SPLIT_ESTABLISHMENT_FAILED':lr==='INCONCLUSIVE'?'SPLIT_VERIFICATION_INCONCLUSIVE':knownTimedPath?'SPLIT_VERIFICATION_REQUIRED':'SPLIT_PATH_UNRESOLVED';
  ...
  /* R10A4 isolation boundary:
     Do NOT patch generateActions() or state(). R10 is projected lazily only
     by r10a4-projection.js when the Actions UI asks for rows. This prevents
     unrelated workflows (including frozen B41 Swarm Control) from executing
     R10 evaluation while loading their own pages. */
  ...
  window.v2p2e5r10OpenVerification=function(actionId){
    ...
    const stage=low(a.workflowStage);
    if(stage==='management-review')return toast('This split needs Queen Management review, not another automatic verification');
    if(stage==='confirmation')return go(`split-queen-path/${encodeURIComponent(txt(a.hiveId))}/${encodeURIComponent(txt(a.sourceActionId))}`);
  ...
  function detailHTML(a){
    ...
    return `<div class="vs v2p2e5r10-detail">
      ...
      <section class="vc"><div class="vhead"><b>Split establishment evidence</b><span class="v2p2e5r10-pill">R10 · S21</span></div><div class="v2p2e5r10-grid">
        ...
        <span>Split completed<b>${esc(e.splitCompletedDate||'Not recorded')}</b></span><span>Queen path<b>${esc(e.queenPlan||'Not decided')}</b></span><span>Path source<b>${esc(pathSourceLabel(e.queenPathSource))}</b></span><span>Actual queen outcome<b>${esc(e.queenOutcome||'Not recorded')}</b></span><span>Planned queen plan<b>${esc(e.plannedQueenPlan||'Not decided')}</b></span><span>Plan / actual conflict<b>${e.queenPlanActualConflict?'Yes — actual result controls':'No'}</b></span>
      ...
      </div>${e.queenPathCode==='QUEEN_CELL'?`<div class="v2p2e5r10-warning"><b>Queen-cell timing boundary</b><br>The old 7-day B39 follow-up is not a final queen-right deadline. Around day 21, begin checking for queen-right evidence. Until the end of the day-28 window, missing eggs / young brood remains unresolved rather than automatic failure.</div>`:`<div class="v2p2e5r10-warning"><b>Path boundary</b><br>HiveDash will not invent a queen-right deadline when the child-hive queen path is not recorded precisely enough. Completed Split Actual Result outranks the earlier plan; if they disagree, confirm the child queen path before timing establishment.</div>`}</section>
      <section class="vc"><div class="vhead"><b>What happens next</b></div><p>${stage==='management-review'?'Review the failed establishment verification and choose Queen Management / R02 only if appropriate.':stage==='confirmation'?'Confirm the child hive queen path first. HiveDash will not calculate or interpret a queen-right verification window until that actual path is known.':beforeStart?'You may record an early establishment check, but absence of eggs or young brood cannot fail this Queen-cell split yet.':'Record targeted split-establishment evidence. This does not count as a Full Inspection.'}</p></section>
      <div class="v2p2e5r10-actions"><button class="secondary" onclick="go('actions')">Back</button>${stage==='management-review'?`<button class="primary" onclick="v2p2e5r10ReviewQueen('${esc(a.id)}')">Review Queen Management</button>`:stage==='confirmation'?`<button class="primary" onclick="v2p2e5r10OpenVerification('${esc(a.id)}')">Confirm Child Queen Path</button>`:`<button class="primary" onclick="v2p2e5r10OpenVerification('${esc(a.id)}')">${beforeStart?'Start Establishment Check':'Start Split Verification'}</button>`}</div>
    </div>`;
  }
  ...
  function formHTML(hid,splitId){
    ...
    const ev=evaluationForSplit(s,a),t=ev.timing,e=ev.task?.evidenceChain||evidenceChain(s,a,child,source,t,null),age=Number(t.daysSinceSplit),early=t.path.code==='QUEEN_CELL'&&age<Number(t.path.startDays),preFinal=t.path.code==='QUEEN_CELL'&&age<Number(t.path.finalDays),finalDay=t.path.code==='QUEEN_CELL'&&age===Number(t.path.finalDays),postFinal=t.path.code==='QUEEN_CELL'&&age>Number(t.path.finalDays);
    return `<div class="vs v2p2e5r10-form-page"><section class="vc"><div class="vhead"><b>Split Verification · targeted evidence</b><span class="v2p2e5r10-pill">R10 · S21</span></div><p class="muted">Record only what you assess now. Split plan values and earlier hive records are reference only. This does not count as a Full Inspection.</p>${early?`<div class="v2p2e5r10-warning"><b>Early establishment check</b><br>This Queen-cell split is only ${esc(t.daysSinceSplit)} days old. Missing eggs / young brood cannot be treated as failure. Final queen-right failure is blocked through the end of day 28.</div>`:preFinal?`<div class="v2p2e5r10-warning"><b>Queen-right verification window</b><br>Current evidence may prove establishment early, but missing eggs / young brood remains unresolved through the end of day 28.</div>`:finalDay?`<div class="v2p2e5r10-warning"><b>Final queen-right window · day 28</b><br>This is the last day of the Queen-cell grace window. Missing eggs / young brood today remains unresolved, not automatic failure.</div>`:postFinal?`<div class="v2p2e5r10-warning"><b>Post-window verification</b><br>The day-28 Queen-cell window has ended. Record complete current evidence. Missing or unassessed fields remain inconclusive; only complete negative queen-right evidence may escalate to Queen Management review.</div>`:''}</section>
  ...
  const pathRouteKey=(hid,splitId)=>`split-queen-path/${txt(hid)}/${txt(splitId)}`;
  function currentPathKey(){
    const p=txt(location.hash||'').replace(/^#/,'').split('/');
    return p[0]==='split-queen-path'&&p[1]&&p[2]?pathRouteKey(decodeURIComponent(txt(p[1])),decodeURIComponent(txt(p[2]))):'';
  }
  function snapshotPathDraft(k=currentPathKey()){
    if(!k)return;
    const choice=document.getElementById('r10-path-choice');
    const date=document.getElementById('r10-intro-date');
    if(!choice&&!date)return;
    const prev=pathDrafts.get(k)||{};
    pathDrafts.set(k,{choice:choice?txt(choice.value):txt(prev.choice),introductionDate:date?txt(date.value):txt(prev.introductionDate)});
  }
  function restorePathDraft(k=currentPathKey()){
    if(!k)return;
    const d=pathDrafts.get(k);if(!d)return;
    const choice=document.getElementById('r10-path-choice');
    const date=document.getElementById('r10-intro-date');
    if(choice&&txt(d.choice))choice.value=txt(d.choice);
    if(date&&txt(d.introductionDate))date.value=txt(d.introductionDate);
    const wrap=document.getElementById('r10-intro-date-wrap');
    if(wrap)wrap.style.display=txt(choice?.value)==='INTRODUCED_QUEEN'?'flex':'none';
  }

  function pathConfirmationHTML(hid,splitId){
    const s=S(),a=splitById(s,splitId),child=hiveBy(s,hid),source=sourceForSplit(s,a);
    if(!a||!child)return `<div class="vs"><section class="vc"><b>Queen Path Confirmation unavailable</b><p>The linked Split or child Hive could not be found.</p><button class="primary" onclick="go('actions')">Back to Actions</button></section></div>`;
    const t=timingFor(s,a,child),e=evidenceChain(s,a,child,source,t,null),latest=latestPathConfirmation(s,splitId),draft=pathDrafts.get(pathRouteKey(hid,splitId))||{};
    const dc=txt(draft.choice),dd=txt(draft.introductionDate);
    return `<div class="vs v2p2e5r10-form-page v2p2e5r10-path-page">
      <section class="vc"><div class="vhead"><b>Confirm Child Queen Path</b><span class="v2p2e5r10-pill">R10 · S21</span></div>
        <p class="muted">Confirm what actually happened in the child hive. The earlier Split plan is reference only and will not preselect an answer.</p>
        <div class="v2p2e5r10-warning"><b>Why confirmation is required</b><br>The completed Split says <b>${esc(e.queenOutcome||'Not recorded')}</b>, which does not define the child hive's queen path precisely enough for a scientific queen-right clock.</div>
      </section>
      <section class="vc"><div class="vhead"><b>Split reference</b></div><div class="v2p2e5r10-grid">
        <span>Source hive<b>${esc(e.sourceHiveName||'Not recorded')}</b></span><span>Child hive<b>${esc(e.childHiveName||'Not recorded')}</b></span>
        <span>Split completed<b>${esc(e.splitCompletedDate||'Not recorded')}</b></span><span>Actual queen outcome<b>${esc(e.queenOutcome||'Not recorded')}</b></span>
        <span>Planned queen plan<b>${esc(e.plannedQueenPlan||'Not decided')}</b></span><span>Current path status<b>${esc(t.phase)}</b></span>
        <span>Old B39 follow-up<b>${esc(e.legacyFollowUpDate||'Not recorded')}</b></span><span>Latest confirmation<b>${esc(latest?.pathLabel||'None')}</b></span>
      </div></section>
      <section class="vc v2p2e5r10-form">
        <label><span>Child Queen Path</span><select id="r10-path-choice" onchange="v2p2e5r10PathChoiceChanged()">
          <option value="" ${!dc?'selected':''}>Select child queen path</option>
          <option value="QUEEN_CELL" ${dc==='QUEEN_CELL'?'selected':''}>Queen cell / colony rears queen</option>
          <option value="LAYING_QUEEN_TRANSFER" ${dc==='LAYING_QUEEN_TRANSFER'?'selected':''}>Existing queen transferred with split</option>
          <option value="INTRODUCED_QUEEN" ${dc==='INTRODUCED_QUEEN'?'selected':''}>Introduced queen</option>
          <option value="UNRESOLVED" ${dc==='UNRESOLVED'?'selected':''}>Unknown / not sure</option>
        </select><small>No option is preselected from the earlier plan.</small></label>
        <label id="r10-intro-date-wrap" style="display:${dc==='INTRODUCED_QUEEN'?'flex':'none'}"><span>Queen Introduction Date</span><input id="r10-intro-date" type="date" value="${esc(dd)}" min="${esc(t.completed||'')}" max="${esc(t.today||'')}" onchange="v2p2e5r10PathIntroDateChanged()"><small>Required only for an introduced queen. HiveDash will time the verification window from this actual introduction date, not the Split date.</small></label>
      </section>
      <div class="v2p2e5r10-actions"><button class="secondary" onclick="go('actions')">Cancel</button><button class="primary" onclick="v2p2e5r10SavePathConfirmation('${esc(hid)}','${esc(splitId)}')">Save Queen Path Confirmation</button></div>
    </div>`;
  }

  window.v2p2e5r10PathChoiceChanged=function(){
    const choice=txt(document.getElementById('r10-path-choice')?.value),wrap=document.getElementById('r10-intro-date-wrap');
    snapshotPathDraft();
    if(wrap)wrap.style.display=choice==='INTRODUCED_QUEEN'?'flex':'none';
  };
  window.v2p2e5r10PathIntroDateChanged=function(){snapshotPathDraft();};
  document.addEventListener('input',ev=>{if(ev?.target?.id==='r10-path-choice'||ev?.target?.id==='r10-intro-date')snapshotPathDraft()},true);
  document.addEventListener('change',ev=>{if(ev?.target?.id==='r10-path-choice'||ev?.target?.id==='r10-intro-date')snapshotPathDraft()},true);

  window.v2p2e5r10SavePathConfirmation=function(hid,splitId){
    const s=S(),a=splitById(s,splitId),child=hiveBy(s,hid);if(!a||!child)return toast('Linked Split is no longer available');
    const draft=pathDrafts.get(pathRouteKey(hid,splitId))||{};
    const choice=txt(document.getElementById('r10-path-choice')?.value)||txt(draft.choice);
    if(!choice)return toast('Select the child queen path');
    const labels={QUEEN_CELL:'Queen cell',LAYING_QUEEN_TRANSFER:'Existing queen transferred with split',INTRODUCED_QUEEN:'Introduced queen',UNRESOLVED:'Unknown / not sure'};
    let introductionDate='';
    if(choice==='INTRODUCED_QUEEN'){
      introductionDate=iso(document.getElementById('r10-intro-date')?.value||draft.introductionDate);
      if(!introductionDate)return toast('Enter the queen introduction date');
      const completed=iso(a?.resultData?.completedDate||a?.completedAt),today=todayFor(s,child);
      if(completed&&introductionDate<completed)return toast('Introduction date cannot be before the Split completion date');
      if(today&&introductionDate>today)return toast('Introduction date cannot be in the future');
    }
    s.meta=s.meta||{};s.meta.splitQueenPathConfirmations=Array.isArray(s.meta.splitQueenPathConfirmations)?s.meta.splitQueenPathConfirmations:[];
    const row={id:'split-queen-path-confirmation-'+Date.now(),splitActionId:txt(splitId),hiveId:txt(hid),childHiveId:txt(hid),sourceHiveId:txt(a.hiveId),pathCode:choice,pathLabel:labels[choice]||choice,introductionDate,date:todayFor(s,child),confirmedAt:new Date().toISOString(),source:'user-confirmed-child-path',coreRuleId:CORE_RULE_ID,ruleVersion:RULE_VERSION,catalogRuleId:CATALOG_RULE_ID,catalogTaskId:CATALOG_TASK_ID};
    s.meta.splitQueenPathConfirmations.push(row);
    if(typeof save==='function'&&save(s)===false)return toast('Queen Path Confirmation could not be saved');
    pathDrafts.delete(pathRouteKey(hid,splitId));toast(choice==='UNRESOLVED'?'Queen path remains unconfirmed':'Queen path confirmed');go('actions');
  };
  ...
  if(typeof prevRender==='function'){
    window.render=function(){
      const before=routeKey();if(before)snap(before);snapshotPathDraft();
      ...
      if(p[0]==='split-queen-path'){
        const hid=decodeURIComponent(txt(p[1])),splitId=decodeURIComponent(txt(p[2])),r=document.getElementById('view');if(!r)return;r.className='view secondary';r.innerHTML=pathConfirmationHTML(hid,splitId);const top=document.getElementById('topbar');if(top){top.className='topbar vtop';top.innerHTML='<button class="iconbtn" onclick="go(\'actions\')" aria-label="Back">‹</button><div class="pagebar-title">Confirm Child Queen Path</div><span></span>'}document.getElementById('bottomnav')?.classList.add('hidden');restorePathDraft(pathRouteKey(hid,splitId));queueMicrotask(()=>restorePathDraft(pathRouteKey(hid,splitId)));setTimeout(()=>restorePathDraft(pathRouteKey(hid,splitId)),0);return;
      }
  ...
  let lastKey=routeKey(),lastPathKey=currentPathKey();window.addEventListener('hashchange',()=>{const next=routeKey();if(lastKey&&!next)drafts.delete(lastKey);lastKey=next;const np=currentPathKey();if(lastPathKey&&lastPathKey!==np)pathDrafts.delete(lastPathKey);lastPathKey=np});

  const style=document.createElement('style');style.id='v2p2e5r10a-style';style.textContent=`
    .v2p2e5r10-pill{display:inline-flex;padding:3px 7px;border-radius:999px;background:#f1f4eb;color:#58704d;font-size:10px;font-weight:700}.v2p2e5r10-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px 14px;margin-top:8px}.v2p2e5r10-grid span{display:flex;flex-direction:column;font-size:11px;color:#667064}.v2p2e5r10-grid b{margin-top:2px;color:#263527;font-size:12px;overflow-wrap:anywhere}.v2p2e5r10-warning{margin-top:12px;padding:10px;border-radius:10px;background:#fff7dc;border:1px solid #eadca7;font-size:11px;line-height:1.45;color:#5b543b}.v2p2e5r10-form label{display:flex;flex-direction:column;gap:6px;padding:10px 0;border-bottom:1px solid #ece8dd}.v2p2e5r10-form label:last-child{border-bottom:0}.v2p2e5r10-form select,.v2p2e5r10-form input[type=date]{width:100%;min-height:44px;border:1px solid #ddd7c8;border-radius:10px;background:#fff;padding:0 10px;box-sizing:border-box}.v2p2e5r10-form small{color:#8d7425;font-size:10px}.v2p2e5r10-note{display:flex;flex-direction:column;gap:8px}.v2p2e5r10-note textarea{min-height:90px;border:1px solid #ddd7c8;border-radius:10px;padding:10px}.v2p2e5r10-actions{display:grid;grid-template-columns:1fr 1.45fr;gap:10px}.v2p2e5r10-form-page{padding-bottom:22px}
  `;document.head.appendChild(style);

  console.log('V2P2E5R10A15 CORE LOADED | path confirmation draft guard + authoritative timing active');
})();
