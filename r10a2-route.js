/* ==============================================================
   HiveDash V2P2E5R10A2 — R10/S21 deployment + open-route closure
...
(()=>{
  'use strict';
  if(window.__HIVEDASH_V2P2E5R10A2__) return;
  window.__HIVEDASH_V2P2E5R10A2__=true;
  window.__HIVEDASH_V2P2E5R10A2_VERSION__='v2p2e5r10a2-r10-open-route-cache-closure';
  ...
  function evaluatorFallback(id){
    ...
    for(const h of hs){
      try{
        const t=window.v2p2e5r10Evaluate?.(h?.id)?.task||null;
  ...
  window.v2p2e5r10a2OpenTask=function(actionId){
  ...
  window.v2p2e5r10a2OpenVerification=function(actionId){
    const a=taskById(actionId);
    if(!a)return typeof toast==='function'?toast('This Split Verification is no longer active'):null;
  ...
  if(typeof prevAM==='function'){
    window.v2p2e5amOpenTask=function(actionId){
      if(taskById(actionId))return window.v2p2e5r10a2OpenTask(actionId);
  ...
  window.v2p2e5r10OpenVerification=function(actionId){
    if(taskById(actionId))return window.v2p2e5r10a2OpenVerification(actionId);
    ...
    const wrappedDraw=function(mode='Pending'){
      ...
      try{
        ...
        buttons.forEach((btn,i)=>{
          ...
          btn.setAttribute('onclick',`v2p2e5r10a2OpenTask('${js(a.id)}')`);
        });
  ...
  console.log('V2P2E5R10A2 LOADED | R10 projected-task open route and cache closure active');
})();
