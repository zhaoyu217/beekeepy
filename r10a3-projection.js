/* ==============================================================
   HiveDash V2P2E5R10A3 — R10/S21 recursion-safe projection closure
...
(()=>{
  'use strict';
  if(window.__HIVEDASH_V2P2E5R10A3__) return;
  window.__HIVEDASH_V2P2E5R10A3__=true;
  window.__HIVEDASH_V2P2E5R10A3_VERSION__='v2p2e5r10a3-recursion-safe-completed-split-projection';
  ...
  function r10Tasks(s){
    ...
    for(const h of hs){
      ...
      try{
        ...
        // IMPORTANT: use the evaluator that accepts the already-loaded state.
        // Calling window.v2p2e5r10Evaluate(h.id) here re-enters state() ->
        // generateActions() -> this projection wrapper and causes an infinite
        // recursion / Maximum call stack error, which blanks unrelated pages
        // such as frozen B41 #swarm-action/new.
        e=typeof core?.evaluateSplitVerification==='function'
          ? core.evaluateSplitVerification(s,h.id)
          : null;
      }catch(_){e=null}
      const t=e?.task;
  ...
  window.v2p2e5r10a1ProjectionAudit=function(hiveId){
    const s=S(),hid=txt(hiveId);
    let e=null;
    try{
      const core=window.HiveDashTaskEngineCoreV1;
      e=typeof core?.evaluateSplitVerification==='function'?core.evaluateSplitVerification(s,hid):null;
    }catch(_){e=null}
    let rows=[];try{rows=(typeof v53ActionRows==='function'?v53ActionRows('Pending'):[]).filter(a=>txt(a?.hiveId)===hid)}catch(_){ }
    return {
      version:window.__HIVEDASH_V2P2E5R10A3_VERSION__,
  ...
  console.log('V2P2E5R10A3 LOADED | recursion-safe completed-Split projection active');
})();
